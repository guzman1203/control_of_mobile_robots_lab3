"""measurementEstimationNoise controller."""
...
def update_robot(trilat=False):
    ...
    print(f"Latest updated robot pose: {robot_pose}")
    #update the direction the robot is facing and print it
    update_direction()
    print(f"Latest updated grid cell: {robot_pose[2]}, Direction: {dir}")
...
def trilateration(colors, dists):
    ...
    smallest = min(dists)
    # distances used for the radiuses
    ds_used = []
    # list of the cylinder objects
    cyls = [y_cyl, r_cyl, g_cyl, b_cyl]
    # append the index of the cylinders to be used
    c_u = []
    ...
    for i in range(len(dists)):
        if dists[i] == smallest:
            smallest_index = i
            break
    # load the distances to use for the trilateration
    for i in range(len(dists)):
        if i != smallest_index:
            ds_used.append(dists[i])
            c_u.append(i)
            
    # use the 3 longest distances for the trilateration     
    # get the ABCDEF for the trilateration calculation
    A, B, C, D, E, F = get_abcdef(cyls[c_u[0]], cyls[c_u[1]], cyls[c_u[2]], ds_used[0], ds_used[1], ds_used[2])
        
    # see if the x y will be able to calculated with the y, r, g, cylinders
    if ((E*A - B*D) == 0 or (B*D - E*A) == 0):
        ds_used [2] = smallest_index
        c_u[2] = smallest_index
        # if it can't trilaterate, redo ABCDEF, with the r, g, b cylinders
        A, B, C, D, E, F = get_abcdef(cyls[c_u[0]], cyls[c_u[1]], cyls[c_u[2]], ds_used[0], ds_used[1], ds_used[2])
    # see if the x y will be able to calculated, if it can't
    # then there isn't another cylinder to be switched out and must
    # return the initial pose to prevent a division by zero error
    if ((E*A - B*D) == 0 or (B*D - E*A) == 0):
